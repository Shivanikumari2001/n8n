...
export class ChromaDbNvrQuery implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'ChromaDB - NVR Query',
		name: 'chromaDbNvrQuery',
		icon: 'file:chroma.svg',
		group: ['transform'],
		version: 1,
		description: 'Query ChromaDB for NVR streaming and recording information using semantic search',
		defaults: { name: 'ChromaDB - NVR Query' },
		inputs: ['main'],
		outputs: ['main'],
		properties: [
			{
			...
			},
			{
				...
				name: 'collectionName',
				type: 'string',
				default: 'nvr_streaming_recording',
				required: true,
				description: 'ChromaDB collection name for NVR information',
			},
			{
				...
				default: '',
				required: true,
				description: 'Natural language query about NVR streaming or recording',
			},
			{
			...
			},
			{
				...
				default: '',
				required: false,
				description: 'Optional metadata filter in JSON format (e.g., {"category": "streaming"})',
			},
		],
	};
					...
					if (error.message?.includes('not found') || error.message?.includes('does not exist') || error.message?.includes('could not be found')) {
						throw new Error(
							`ChromaDB Error: Collection '${collectionName}' does not exist. ` +
								`Please run populate_chromadb_nvr.py to create and populate the collection.`
						);
					}
